...
namespace Clear2
{
    public class ActionsClear3
    {
        ...
        private static void ReadConfig(string configFilename)
        {
            ...
            projectsFolder = (string)clearSettings["ProjectsFolder"]; // e.g. 
        ...
        // Copy initial state files for a fresh start in processing
        // Decided not to juse copy all the files in this folder but rather specify them one by one based upon the name in the config file.
        // This will guarantee we are copying files that will be the proper name (i.e. if we forget to change file names in the initial folder.
        // string[] initialFiles = Directory.GetFiles(initialFolder);
        public static string Do_Button10()
        {
            string copiedFiles = "Copied Initial State Files:\n";

            copiedFiles = CopyInitialStateFile(transModelFile, copiedFiles);
            copiedFiles = CopyInitialStateFile(alignModelFile, copiedFiles);
            copiedFiles = CopyInitialStateFile(stopWordFile, copiedFiles);
            copiedFiles = CopyInitialStateFile(badLinkFile, copiedFiles);
            copiedFiles = CopyInitialStateFile(goodLinkFile, copiedFiles);
            copiedFiles = CopyInitialStateFile(groupFile, copiedFiles);
            copiedFiles = CopyInitialStateFile(strongFile, copiedFiles);
            copiedFiles = CopyInitialStateFile(oldJson, copiedFiles);
            copiedFiles = CopyInitialStateFile(t2gJsonFile, copiedFiles);
            copiedFiles = CopyInitialStateFile(checkedAlignmentsFile, copiedFiles);
            copiedFiles = CopyInitialStateFile(manTransModelFile, copiedFiles);
            copiedFiles = CopyInitialStateFile(tmFile, copiedFiles);
        ...
        private static string CopyInitialStateFile(string targetFile, string copiedFiles)
        {
        ...
        static void WriteTransModelSorted(TranslationModel model, string file)
        {
            ...
            {
                string[] parts = entry.Key.Split("\t".ToCharArray());
        ...
        private static string projectsFolder; // 2020.07.11 CL: The folder where all the CLEAR Engine translation projects are kept.
